...
the srcFs:srcRemote must point to a text file in the SUM format.
...
func rcCheck(ctx context.Context, in rc.Params) (out rc.Params, err error) {
	...
	if checkFileHash != "" {
		...
		err = CheckSum(context.Background(), srcFs, checkFileFs, checkFileRemote, checkFileHashType, opt, download)
	} else {
		if download {
